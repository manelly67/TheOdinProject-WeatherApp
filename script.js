...
header.setAttribute(
  "style",
  "font-size: 2rem;display: flex;justify-content: center;text-transform: uppercase;"
);
...
searchInput.setAttribute(
  "pattern",
  "^[a-zA-Z0-9áéíóúÁÉÍÓÚñÑüÜàÀâÂãÃçÇêÊôÔõÕ ]+$"
);
...
const baseUrl = "https://api.weatherapi.com/v1/current.json?key=8e1ecc57edb74e0a8ca192317240506&q=";
let weatherapiUrl = baseUrl;
...
const readData = (arg) => {
  ...
  let weatherData = arg;
  let data = {
    place: `${weatherData.location.name.toUpperCase()} - ${
...
async function getWeatherData(arg) {
  ...
  try {
    ...
    alert("Something was wrong or the location doesn't exists. Try again later");
  }
}
...
function displayData(data) {
  ...
  place.setAttribute(
    "style",
    "grid-area: 1 / 1 / 2 / 3; font-size: 2rem;display:flex ;justify-content: center;"
  );
  country.innerText = `${data.country}`;
  country.setAttribute(
    "style",
    "grid-area: 2 / 1 / 3 / 2;display:flex ;justify-content: center;"
  );
  date.innerText = `date: ${data.date.slice(0, 10)} time: ${data.date.slice(
    11,
    16
  )}`;
  ...
  divTemp.setAttribute(
    "style",
    "grid-area: 3/1/4/3;display:flex;justify-content: space-around;flex-wrap: nowrap;"
  );
  ...
  divWindRain.setAttribute(
    "style",
    "grid-area: 4/1/5/3;display:flex;justify-content: space-around;flex-wrap: nowrap;"
  );
  wind.innerText = `Wind: ${data.c_kph_mm.wind} Kph`;
  precip.innerText = `Precipitation: ${data.c_kph_mm.precip} millimetres`;
  divCloudUv.setAttribute(
    "style",
    "grid-area: 5/1/6/3;display:flex;justify-content: space-around;flex-wrap: nowrap;"
  );
...
function updateBackground(data) {
  if (data.c_kph_mm.temp <= 6) {
    container.classList.add("cold-weather");
  } else {
    if ((data.c_kph_mm.temp > 6) & (data.c_kph_mm.temp < 30)) {
